 ...
 *    down to the target's directory, then the global `~/.agentignore` last
...
interface IgnoreFile {
	...
	matcher: Ignore;
	patternCount: number;
	/** The global ~/.agentignore: matches against full absolute path segments
	 *  so it applies to paths outside its own directory too. */
	global?: boolean;
}
...
export class IgnoreEngine {
	...
	constructor(
		globalPath = path.join(os.homedir(), LOCAL_FILE_NAME),
	...
	/** The .agentignore files currently in scope for cwd (for /agentignore). */
...
/** Number of real patterns in a rules document (for /agentignore display). */
